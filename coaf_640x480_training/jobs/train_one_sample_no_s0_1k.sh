#!/usr/bin/env bash
# One-sample 640x480 overfit for the no-state0 v6 model.
...
export DATA_ROOT="${DATA_ROOT:-${ONE_SAMPLE_640_DATA_ROOT}}"
export OUTPUT_DIR="${OUTPUT_DIR:-${CASUAL_ROOT}/outputs/checkpoints/i2av/v6_depth_track_rgb_640x480_one_sample_no_s0_1k}"
...
export TRACKER_NAME="${TRACKER_NAME:-casual-coaf-i2av-v6-640x480-one-sample-no-s0-1k}"
...
bash "${CASUAL_ROOT}/scripts/train_cogvideox_i2av_lora_causal.sh" \
  2>&1 | tee "${CASUAL_ROOT}/logs/train/i2av/local_v6_640x480_one_sample_no_s0_1k_$(date +%Y%m%d_%H%M%S).log"
