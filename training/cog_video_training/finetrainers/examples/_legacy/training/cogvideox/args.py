...
def _get_dataset_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional column or file containing condition images for image-to-video training.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
...
def _get_training_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use temporal causal self-attention over video tokens in CogVideoX DiT blocks.",
    )
    parser.add_argument(
        "--enable_i2av",
        action="store_true",
        help="Enable joint image-to-action-video training with state/action tokens.",
    )
    parser.add_argument(
        "--state_norm_stats",
        type=str,
    ...
    parser.add_argument(
        "--stage2_clean_past_sa",
        action="store_true",
        help="For stage2 SA denoise, replace past SA chunks with clean GT tokens and train only a random target chunk.",
    )
    parser.add_argument("--sa_per_frame", type=int, default=8)
    parser.add_argument("--s0_cond_tokens", type=int, default=0)
    parser.add_argument("--i2av_layout", choices=["legacy", "v5", "v6"], default="legacy")
    parser.add_argument(
        "--action_chunk_alignment",
        choices=["current", "next_window"],
        default="current",
        help=(
            "For v5 chunked SA tokens, keep legacy current-window action alignment or align "
            "each chunk's actions as the control plan for the next latent window."
        ),
    )
    ...
    parser.add_argument(
        "--direct_action_head",
        action="store_true",
        help="Enable the new v5 direct continuous action head path instead of SA token denoising/decoding.",
    )
    parser.add_argument(
        "--direct_action_use_past_action_cond",
        action="store_true",
        help="Condition pose chunk P_k on past action condition tokens A_<k in the direct-action path.",
    )
    parser.add_argument("--direct_action_horizon", type=int, default=25)
    parser.add_argument("--lambda_direct_action", type=float, default=1.0)
    parser.add_argument("--track_norm_stats", type=str, default=None)
    parser.add_argument("--track_column", type=str, default=None)
    parser.add_argument("--track_visible_column", type=str, default=None)
