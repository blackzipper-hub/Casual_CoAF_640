...
def prepare_i2av_rotary_positional_embeddings(
    height: int,
    width: int,
    num_frames: int,
    vae_scale_factor_spatial: int = 8,
    ...
    if freqs_cos.shape[0] % patches_per_frame != 0:
        raise ValueError(
            "Unexpected CogVideoX RoPE length: "
            f"{freqs_cos.shape[0]} is not divisible by {patches_per_frame} patches/frame"
        )
    # ``num_frames`` here is already the transformer latent-frame count. Infer
    # from the base RoPE tensor to avoid applying temporal compression twice.
    num_latent_frames = freqs_cos.shape[0] // patches_per_frame
    return expand_rope_for_i2av(
        freqs_cos, freqs_sin, num_latent_frames, patches_per_frame, sa_per_frame
    )
...
def prepare_i2av_v5_direct_action_rotary_positional_embeddings(
    height: int,
    width: int,
    layout,
    vae_scale_factor_spatial: int = 8,
    ...
    from finetrainers.patches.models.cogvideox.i2av_sequence import expand_rope_for_direct_action_i2av_timeids
    ...
    expected = (2 * layout.num_pose_latent_frames + layout.num_rgb_latent_frames) * layout.patches_per_frame
    if freqs_cos.shape[0] != expected:
        raise ValueError(f"Unexpected direct-action v5 RoPE length: got {freqs_cos.shape[0]}, expected {expected}.")
...
def prepare_i2av_v6_rotary_positional_embeddings(
    height: int,
    width: int,
    layout,
    vae_scale_factor_spatial: int = 8,
    ...
    from finetrainers.patches.models.cogvideox.i2av_sequence import expand_rope_for_v6_i2av_timeids

    freqs_cos, freqs_sin = prepare_rotary_positional_embeddings(
        height=height,
        width=width,
        num_frames=max(layout.num_latent_frames, layout.track_steps),
        vae_scale_factor_spatial=vae_scale_factor_spatial,
        patch_size=patch_size,
        patch_size_t=patch_size_t,
        attention_head_dim=attention_head_dim,
