...
from .i2av_layout import I2AVV5Layout, I2AVV6Layout, compute_i2av_v5_layout, compute_i2av_v6_layout
...
class CausalAttentionMeta:
    ...
    v6_layout: dict[str, Any] | None = None
...
class CogVideoXI2AVV5DirectActionCausalAttnProcessor2_0(CogVideoXCausalTemporalAttnProcessor2_0):
    """v5 direct-action mask for [P0,A0_t,P1,A1_t,...,P6,A6_t,RGB].

    P_k sees only A_<k to avoid current-action leakage. A_k_t sees itself
    because it is the noised diffusion token being denoised.
    """
    ...
    @property
    def direct_pose_tokens(self) -> int:
        return self.layout.num_pose_latent_frames * (self.layout.patches_per_frame + 1)

    @property
    def direct_video_tokens(self) -> int:
        return self.direct_pose_tokens + self.layout.rgb_video_tokens
    ...
    def _temporal_causal_attention(
        ...
        if text_seq_length != self.layout.condition_tokens:
            raise ValueError(
                f"I2AV direct-action condition length mismatch: got {text_seq_length}, "
                f"expected {self.layout.condition_tokens}."
            )
        video_seq_length = query.size(2) - text_seq_length
        if video_seq_length != self.direct_video_tokens:
            raise ValueError(
                f"Unexpected I2AV direct-action token count: got {video_seq_length}, "
                f"expected {self.direct_video_tokens}."
            )
        if self.backend == "flex":
            raise NotImplementedError("I2AV direct-action FlexAttention mask is not implemented yet; use chunked backend.")
        ...
        cursor = pose_base
        for chunk_idx in range(self.layout.num_pose_latent_frames):
            q_pose = query[:, :, cursor : cursor + p]
            # P_k sees condition, P_<=k and A_<k. This excludes A_k.
            pose_kv_end = cursor + p
            outputs.append(
                F.scaled_dot_product_attention(
                    q_pose,
                    key[:, :, :pose_kv_end],
            ...
            q_action = query[:, :, cursor : cursor + 1]
            # A_k_t sees condition, P_<=k, A_<k and itself, then predicts epsilon_k.
            action_kv_end = cursor + 1
            outputs.append(
                F.scaled_dot_product_attention(
                    q_action,
                    key[:, :, :action_kv_end],
        ...
        rgb_start = cursor
        rgb_outputs = []
        for rgb_idx in range(self.layout.num_rgb_latent_frames):
            q_start = rgb_start + rgb_idx * p
            q_end = q_start + p
            # RGB_j sees condition, all pose/action-cond tokens, and RGB_<=j.
            rgb_outputs.append(
                F.scaled_dot_product_attention(
                    query[:, :, q_start:q_end],
                    key[:, :, :q_end],
        ...
        outputs.extend(rgb_outputs)
        return torch.cat([condition_output, *outputs], dim=2)


class CogVideoXI2AVV6BlockCausalAttnProcessor2_0(CogVideoXCausalTemporalAttnProcessor2_0):
    """v6 block mask for condition + bidirectional reason + bidirectional RGB."""

    def __init__(self, layout: I2AVV6Layout):
        super().__init__(num_latent_frames=layout.num_latent_frames, patches_per_frame=layout.patches_per_frame)
        self.layout = layout
    ...
    def _temporal_causal_attention(
        ...
        if text_seq_length != self.layout.condition_tokens:
            raise ValueError(
                f"I2AV v6 condition length mismatch: got {text_seq_length}, "
                f"expected {self.layout.condition_tokens}."
            )
        video_seq_length = query.size(2) - text_seq_length
        if video_seq_length != self.layout.video_tokens:
            raise ValueError(
                f"Unexpected I2AV v6 token count: got {video_seq_length}, expected {self.layout.video_tokens}."
            )
        if self.backend == "flex":
            raise NotImplementedError("I2AV v6 FlexAttention mask is not implemented yet; use chunked backend.")

        reason_start = text_seq_length
        reason_end = reason_start + self.layout.reason_tokens
        rgb_start = reason_end
        rgb_end = rgb_start + self.layout.rgb_video_tokens

        # Conditioning tokens are global; reason sees condition+reason; RGB sees all prior reason plus RGB.
        condition_output = F.scaled_dot_product_attention(
            query[:, :, :text_seq_length],
            key[:, :, :rgb_end],
            value[:, :, :rgb_end],
        ...
        reason_output = F.scaled_dot_product_attention(
            query[:, :, reason_start:reason_end],
            key[:, :, :reason_end],
            value[:, :, :reason_end],
            dropout_p=0.0,
            is_causal=False,
        ...
        return torch.cat([condition_output, reason_output, rgb_output], dim=2)
    ...
    *,
    ...
    if enable_state_action and i2av_layout == "v5":
        ...
        if direct_action_head:
            processor = CogVideoXI2AVV5DirectActionCausalAttnProcessor2_0(v5_layout)
            tokens_per_step = v5_layout.patches_per_frame + 1
            video_seq_length = v5_layout.num_pose_latent_frames * tokens_per_step
            video_seq_length += v5_layout.rgb_video_tokens
        else:
    ...
    elif enable_state_action and i2av_layout == "v6":
        base_text_seq_length = (
            text_seq_length - s0_cond_tokens
            if text_seq_length > transformer.config.max_text_seq_length
            else text_seq_length
        ...
        v6_layout = compute_i2av_v6_layout(
            ...
            vae_scale_factor_spatial=vae_scale_factor_spatial,
            temporal_compression_ratio=temporal_compression_ratio,
            track_steps=pose_pixel_frames,
            track_tokens_per_step=1,
        )
        processor = CogVideoXI2AVV6BlockCausalAttnProcessor2_0(v6_layout)
        tokens_per_step = v6_layout.track_tokens_per_step
        video_seq_length = v6_layout.video_tokens
    ...
    meta = CausalAttentionMeta(
        ...
        num_pose_latent_frames=(
            v5_layout.num_pose_latent_frames
            if v5_layout is not None
            else v6_layout.num_pose_latent_frames
            if v6_layout is not None
            else 0
        ),
        num_rgb_latent_frames=(
            v5_layout.num_rgb_latent_frames
            if v5_layout is not None
            else v6_layout.num_rgb_latent_frames
            if v6_layout is not None
            else 0
        ),
        chunk_token_count=v5_layout.chunk_token_count if v5_layout is not None else 0,
        pose_pixel_frames=pose_pixel_frames if (v5_layout is not None or v6_layout is not None) else 0,
        rgb_pixel_frames=rgb_pixel_frames if (v5_layout is not None or v6_layout is not None) else 0,
        v5_layout=v5_layout.to_dict() if v5_layout is not None else None,
        v6_layout=v6_layout.to_dict() if v6_layout is not None else None,
    )
    if v5_layout is not None:
        transformer._coaf_i2av_v5_layout = v5_layout  # noqa: SLF001
    if v6_layout is not None:
        transformer._coaf_i2av_v6_layout = v6_layout  # noqa: SLF001
