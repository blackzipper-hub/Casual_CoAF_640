...
@dataclass(frozen=True)
class I2AVV6Layout:
    """Runtime-derived token layout for v6 depth + track + RGB blocks."""
    ...
    @property
    def pose_video_tokens(self) -> int:
        return self.num_pose_latent_frames * self.patches_per_frame

    @property
    def track_tokens(self) -> int:
        return self.track_steps * self.track_tokens_per_step

    @property
    def reason_tokens(self) -> int:
        return self.pose_video_tokens + self.track_tokens
    ...
    @property
    def video_tokens(self) -> int:
        return self.reason_tokens + self.rgb_video_tokens

    @property
    def visual_tokens(self) -> int:
        return self.num_latent_frames * self.patches_per_frame
...
def compute_i2av_v6_layout(
    ...
    *,
    ...
    if num_rgb_latent_frames <= 0:
        raise ValueError(
            f"Bad v6 frame split: pose={pose_pixel_frames}, rgb={rgb_pixel_frames} "
            f"yields F_pose={num_pose_latent_frames}, F_rgb={num_rgb_latent_frames}."
        )
    patches_per_frame = patches_per_latent_frame(
        pixel_height=pixel_height,
    ...
    return I2AVV6Layout(
        ...
        pixel_height=int(pixel_height),
        pixel_width=int(pixel_width),
        track_steps=int(track_steps or pose_pixel_frames),
        track_tokens_per_step=int(track_tokens_per_step),
    )
