...
def build_v6_depth_track_rgb_tokens(
    pose_visual_tokens: torch.Tensor,
    track_tokens: torch.Tensor,
    rgb_visual_tokens: torch.Tensor,
) -> torch.Tensor:
    """v6 layout: [Depth/Pose latents, Track tokens, RGB latents]."""
    if track_tokens.ndim != 3:
        raise ValueError(f"Expected track tokens shape (B,T,D), got {tuple(track_tokens.shape)}")
    return torch.cat([pose_visual_tokens, track_tokens, rgb_visual_tokens], dim=1)


def deinterleave_v6_depth_track_rgb_tokens(
    tokens: torch.Tensor,
    *,
    num_pose_latent_frames: int,
    num_rgb_latent_frames: int,
    ...
    """Split v6 sequence back into pose visual, track, and rgb visual tokens."""
    pose_len = num_pose_latent_frames * patches_per_frame
    rgb_len = num_rgb_latent_frames * patches_per_frame
    pose_visual = tokens[:, :pose_len]
    track = tokens[:, pose_len : pose_len + track_tokens]
    rgb_visual = tokens[:, pose_len + track_tokens : pose_len + track_tokens + rgb_len]
    if rgb_visual.shape[1] != rgb_len:
        raise ValueError(f"Unexpected v6 RGB token count: got {rgb_visual.shape[1]}, expected {rgb_len}.")
    return pose_visual, track, rgb_visual
...
def expand_rope_for_direct_action_i2av_timeids(
    freqs_cos: torch.Tensor,
    freqs_sin: torch.Tensor,
    num_pose_latent_frames: int,
    num_rgb_latent_frames: int,
    ...
    """RoPE for direct-action v5: P_k=2k, A_k_t=2k+1, R_j=2F_pose+j."""
    cos_parts: list[torch.Tensor] = []
    sin_parts: list[torch.Tensor] = []
    for t in range(num_pose_latent_frames):
        pose_start = (2 * t) * patches_per_frame
        ...
        cos_parts.append(freqs_cos[pose_start:pose_end])
        sin_parts.append(freqs_sin[pose_start:pose_end])
        action_start = (2 * t + 1) * patches_per_frame
        action_end = action_start + patches_per_frame
        cos_parts.append(freqs_cos[action_end - 1 : action_end])
        sin_parts.append(freqs_sin[action_end - 1 : action_end])
    rgb_base = 2 * num_pose_latent_frames * patches_per_frame
    rgb_len = num_rgb_latent_frames * patches_per_frame
    cos_parts.append(freqs_cos[rgb_base : rgb_base + rgb_len])
    sin_parts.append(freqs_sin[rgb_base : rgb_base + rgb_len])
...
def expand_rope_for_v6_i2av_timeids(
    freqs_cos: torch.Tensor,
    freqs_sin: torch.Tensor,
    *,
    num_pose_latent_frames: int,
    ...
    patches_per_frame: int,
    track_steps: int,
    track_tokens_per_step: int = 1,
) -> tuple[torch.Tensor, torch.Tensor]:
    """RoPE for v6: Depth L0-L6, Track T0-T24, RGB L7-L12."""
    cos_parts: list[torch.Tensor] = []
    sin_parts: list[torch.Tensor] = []

    pose_len = num_pose_latent_frames * patches_per_frame
    cos_parts.append(freqs_cos[:pose_len])
    sin_parts.append(freqs_sin[:pose_len])

    max_time_rows = freqs_cos.shape[0] // patches_per_frame
    for t in range(track_steps):
        time_idx = min(t, max_time_rows - 1)
        row_start = time_idx * patches_per_frame
        row = freqs_cos[row_start + patches_per_frame - 1 : row_start + patches_per_frame]
        sin_row = freqs_sin[row_start + patches_per_frame - 1 : row_start + patches_per_frame]
        cos_parts.append(row.repeat(track_tokens_per_step, 1))
        sin_parts.append(sin_row.repeat(track_tokens_per_step, 1))

    rgb_base = num_pose_latent_frames * patches_per_frame
    rgb_len = num_rgb_latent_frames * patches_per_frame
    cos_parts.append(freqs_cos[rgb_base : rgb_base + rgb_len])
    sin_parts.append(freqs_sin[rgb_base : rgb_base + rgb_len])
    return torch.cat(cos_parts, dim=0), torch.cat(sin_parts, dim=0)
