#!/usr/bin/env bash
# Local one-sample overfit for CoAF v6 depth+track+RGB at 640x480.
...
export DATA_ROOT="${DATA_ROOT:-${PROJECT_ROOT}/coaf_dataset_24_25/composed/v6_depth_track_rgb_640x480_one_sample_overfit}"
export OUTPUT_DIR="${OUTPUT_DIR:-${CASUAL_ROOT}/outputs/checkpoints/i2av/v6_depth_track_rgb_640x480_one_sample_1k}"
...
export CHECKPOINTING_STEPS="${CHECKPOINTING_STEPS:-200}"
export CHECKPOINTS_TOTAL_LIMIT="${CHECKPOINTS_TOTAL_LIMIT:-8}"
export LR="${LR:-1e-4}"
export LR_WARMUP_STEPS="${LR_WARMUP_STEPS:-10}"

export TRACKER_NAME="${TRACKER_NAME:-casual-coaf-i2av-v6-depth-track-rgb-640x480-one-sample-1k}"
...
bash "${CASUAL_ROOT}/scripts/train_cogvideox_i2av_lora_causal.sh" 2>&1 | tee "${CASUAL_ROOT}/logs/train/i2av/local_v6_depth_track_rgb_640x480_one_sample_1k_$(date +%Y%m%d_%H%M%S).log"
