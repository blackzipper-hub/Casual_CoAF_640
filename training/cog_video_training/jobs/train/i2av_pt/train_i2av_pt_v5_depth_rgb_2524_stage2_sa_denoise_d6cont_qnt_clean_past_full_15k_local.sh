#!/usr/bin/env bash
# Local 4090 launch (no Slurm) for full-data clean past-SA training to 15k steps.
...
export TRACKER_NAME=casual-coaf-i2av-pt-v5-depth-rgb-2524-stage2-sa-denoise-d6cont-qnt-clean-past-full-15k
...
nvidia-smi || true
bash "${CASUAL_ROOT}/scripts/train_cogvideox_i2av_lora_causal.sh" 2>&1 | tee "${CASUAL_ROOT}/logs/train/i2av_pt/local_qnt_clean_past_full_15k_$(date +%Y%m%d_%H%M%S).log"
