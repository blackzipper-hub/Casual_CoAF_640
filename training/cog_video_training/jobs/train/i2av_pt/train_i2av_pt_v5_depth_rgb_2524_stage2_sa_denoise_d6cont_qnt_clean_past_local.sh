#!/usr/bin/env bash
# Local 4090 launch (no Slurm) for stage2 SA denoise + quantile action norm
# with clean past-SA teacher forcing. Each sample picks one target SA chunk;
# chunks before it are clean GT, and SA denoise loss is applied only to the
# target chunk.
#
# Usage:
#   bash jobs/train/i2av_pt/train_i2av_pt_v5_depth_rgb_2524_stage2_sa_denoise_d6cont_qnt_clean_past_local.sh
#   TRAIN_STEPS=10 bash jobs/train/i2av_pt/train_i2av_pt_v5_depth_rgb_2524_stage2_sa_denoise_d6cont_qnt_clean_past_local.sh
#   CUDA_VISIBLE_DEVICES=1 bash jobs/train/i2av_pt/train_i2av_pt_v5_depth_rgb_2524_stage2_sa_denoise_d6cont_qnt_clean_past_local.sh
...
export OUTPUT_DIR="${CASUAL_ROOT}/outputs/checkpoints/i2av/v5_depth_rgb_2524_stage2_sa_denoise_d6cont_qnt_clean_past"
...
export TRACKER_NAME=casual-coaf-i2av-pt-v5-depth-rgb-2524-stage2-sa-denoise-d6cont-qnt-clean-past
...
nvidia-smi || true
bash "${CASUAL_ROOT}/scripts/train_cogvideox_i2av_lora_causal.sh" 2>&1 | tee "${CASUAL_ROOT}/logs/train/i2av_pt/local_qnt_clean_past_$(date +%Y%m%d_%H%M%S).log"
