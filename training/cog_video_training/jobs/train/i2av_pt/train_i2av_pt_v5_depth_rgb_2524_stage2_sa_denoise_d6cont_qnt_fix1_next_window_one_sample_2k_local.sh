#!/usr/bin/env bash
# Local 4090 launch for one-sample overfit on normal v4_depth_rgb data:
# fix1 SA denoise + quantile action norm + next-window action alignment.
# This intentionally does NOT enable clean-past chunk teacher forcing.
...
export TINY_DATA_ROOT="${DATASET_ROOT}/composed/v4_depth_rgb_one_sample_next_window_qnt_fix1"
python - <<'PY'
import json
import os
...
export OUTPUT_DIR="${OUTPUT_DIR:-${CASUAL_ROOT}/outputs/checkpoints/i2av/v5_depth_rgb_2524_stage2_sa_denoise_d6cont_qnt_fix1_next_window_one_sample_2k}"
...
export TRAIN_STEPS="${TRAIN_STEPS:-2000}" CHECKPOINTING_STEPS="${CHECKPOINTING_STEPS:-200}" CHECKPOINTS_TOTAL_LIMIT=12
export LR="${LR:-1e-4}" LR_WARMUP_STEPS="${LR_WARMUP_STEPS:-20}"
export TRACKER_NAME="${TRACKER_NAME:-casual-coaf-i2av-pt-v5-depth-rgb-2524-stage2-sa-denoise-d6cont-qnt-fix1-next-window-one-sample}"
...
nvidia-smi || true
bash "${CASUAL_ROOT}/scripts/train_cogvideox_i2av_lora_causal.sh" 2>&1 | tee "${CASUAL_ROOT}/logs/train/i2av_pt/local_qnt_fix1_next_window_one_sample_$(date +%Y%m%d_%H%M%S).log"
