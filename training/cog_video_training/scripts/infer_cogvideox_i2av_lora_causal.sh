...
if [[ -n "${SIMPLER_DATA_ROOT:-}" ]]; then
  I2AV_EXTRA_ARGS+=(--simpler_data_root "${SIMPLER_DATA_ROOT}")
fi
if [[ -n "${SIMPLER_NUM_SAMPLES:-}" ]]; then
  I2AV_EXTRA_ARGS+=(--simpler_num_samples "${SIMPLER_NUM_SAMPLES}")
fi
if [[ -n "${ACTION_NORM_STATS:-}" ]]; then
  I2AV_EXTRA_ARGS+=(--action_norm_stats "${ACTION_NORM_STATS}")
fi
if [[ -n "${TRACK_NORM_STATS:-}" ]]; then
  I2AV_EXTRA_ARGS+=(--track_norm_stats "${TRACK_NORM_STATS}")
fi
...
if [[ "${DIRECT_ACTION_HEAD:-0}" == "1" ]]; then
  I2AV_EXTRA_ARGS+=(--direct_action_head)
fi
if [[ -n "${DIRECT_ACTION_HORIZON:-}" ]]; then
  I2AV_EXTRA_ARGS+=(--direct_action_horizon "${DIRECT_ACTION_HORIZON}")
fi
...
if [[ "${SIMPLER_SIM_REPLAY:-0}" == "1" ]]; then
  I2AV_EXTRA_ARGS+=(--simpler_sim_replay)
fi
if [[ -n "${SIMPLER_ROOT:-}" ]]; then
  I2AV_EXTRA_ARGS+=(--simpler_root "${SIMPLER_ROOT}")
fi
if [[ -n "${SIMPLER_ACTION_FORMAT:-}" ]]; then
  I2AV_EXTRA_ARGS+=(--simpler_action_format "${SIMPLER_ACTION_FORMAT}")
fi
