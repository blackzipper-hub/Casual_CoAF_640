#!/usr/bin/env python3
"""Precompute CogVideoX I2AV video/image VAE latents in-place.

This is the visual-only companion to precompute_i2av_latents.py. It skips T5
prompt embedding computation so prompt_embeds can be reused via symlink when
the text prompts are unchanged.
"""
...
def main() -> None:
    ...
    progress = tqdm(list(batched(indices, args.batch_size)), desc=f"visual latents shard {args.shard_index}/{args.num_shards}")
    ...
    print(f"Completed visual latents shard {args.shard_index}/{args.num_shards} under {data_root}")
