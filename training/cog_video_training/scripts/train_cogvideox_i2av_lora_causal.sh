...
if [[ "${I2AV_LAYOUT:-legacy}" == "v6" ]]; then
  for required_file in track_paths.txt track_visible_paths.txt; do
    if [[ ! -f "${DATA_ROOT}/${required_file}" ]]; then
      echo "Missing ${DATA_ROOT}/${required_file}" >&2
      exit 1
    fi
  done
  TRACK_NORM_STATS="${TRACK_NORM_STATS:-${DATA_ROOT}/track_norm_stats.pt}"
  if [[ ! -f "${TRACK_NORM_STATS}" ]]; then
    echo "Missing track norm stats at ${TRACK_NORM_STATS}" >&2
    exit 1
  fi
fi
...
if [[ "${STAGE2_CLEAN_PAST_SA:-0}" == "1" ]]; then
  STAGE_ARGS+=(--stage2_clean_past_sa)
fi
ACTION_CHUNK_ALIGNMENT="${ACTION_CHUNK_ALIGNMENT:-current}"
DIRECT_ACTION_ARGS=()
if [[ "${DIRECT_ACTION_HEAD:-0}" == "1" ]]; then
  DIRECT_ACTION_ARGS+=(--direct_action_head)
fi
if [[ "${DIRECT_ACTION_USE_PAST_ACTION_COND:-0}" == "1" ]]; then
  DIRECT_ACTION_ARGS+=(--direct_action_use_past_action_cond)
fi
TRACK_ARGS=()
if [[ "${I2AV_LAYOUT:-legacy}" == "v6" ]]; then
  TRACK_ARGS+=(--track_norm_stats "${TRACK_NORM_STATS}" --track_column track_paths.txt --track_visible_column track_visible_paths.txt)
fi
...
"${LAUNCHER[@]}" cogvideox_image_to_video_lora_i2av.py \
  ...
  --direct_action_horizon "${DIRECT_ACTION_HORIZON:-25}" \
  --lambda_direct_action "${LAMBDA_DIRECT_ACTION:-1.0}" \
